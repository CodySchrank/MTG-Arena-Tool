...
//
function addCardTile(grpId, quantity, element) {
	var cont = $('<div class="card_tile_container"></div>');
	cont.append('<div class="card_tile_quantity"><span>'+quantity+'</span></div>');
	element.append(cont);
	
	var div = $('<div id="t'+grpId+'" class="card_tile '+get_frame_class(database[grpId].frame)+'"></div>');
	cont.append(div);

	// Glow hover
	var glow = $('<div id="t'+grpId+'" class="card_tile_glow"></div>');
	cont.append(glow);
